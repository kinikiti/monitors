...
def main():
    ...
    if (cp4d_platform_global_connections_request.status_code == 200):
        ...
        # Define metadata
        metadata_number_of_global_connections = "{}={}".format(event_type_number_of_connections,
                                                               str(cp4d_number_of_global_connections))

        # Define the global connections count event
        data = {"monitor_type": monitor_type, "event_type": event_type_number_of_connections,
                "metadata": metadata_number_of_global_connections, "severity": "info",
                "reference": "Cloud Pak for Data Global Connections Count"}

        # Add the event to the list of events
        events.append(data)

    else:
        print("Get Connections returned status code: {}. Response: {}".format(
            cp4d_platform_global_connections_request.status_code, cp4d_platform_global_connections_request.text))
        ...
        # Define the global connections count event
        data = {"monitor_type": monitor_type, "event_type": event_type_number_of_connections,
                "metadata": metadata_number_of_global_connections, "severity": "warning",
                "reference": "Cloud Pak for Data Global Connections Count"}
    ...
    if cp4d_number_of_global_connections > 0:
        ...
        for global_connection in cp4d_platform_global_connections_response["resources"]:
            ...
            metadata_valid_connection = f"{event_type_valid_connection}={str(cp4d_platform_connection_valid)}"
            data = {"monitor_type": monitor_type, "event_type": event_type_valid_connection,
                    "metadata": metadata_valid_connection, "severity": severity,
                    "reference": "Global Connection - {}".format(resource_name)}
            events.append(data)
