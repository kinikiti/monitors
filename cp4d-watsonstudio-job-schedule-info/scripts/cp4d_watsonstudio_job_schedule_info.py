...
def main():
    ...
    event_type_watsonstudio_jobs_schedule_overall_count = cp4d_monitor.create_and_validate_type(
        "cp4d_watsonstudio_jobs_schedule_overall_count")
    event_type_watsonstudio_jobs_schedule_project_count = cp4d_monitor.create_and_validate_type(
        "cp4d_watsonstudio_jobs_schedule_project_count")
    event_type_watsonstudio_jobs_schedule_user_overall_count = cp4d_monitor.create_and_validate_type(
        "cp4d_watsonstudio_jobs_schedule_user_overall_count")
    event_type_watsonstudio_job_schedule_last_run_fail = cp4d_monitor.create_and_validate_type(
        "cp4d_watsonstudio_job_schedule_last_run_fail")
    event_type_watsonstudio_job_schedule_next_run_epoch_time = cp4d_monitor.create_and_validate_type(
        "cp4d_watsonstudio_job_schedule_next_run_epoch_time")
    event_type_watsonstudio_job_schedule_last_run_duration_seconds = cp4d_monitor.create_and_validate_type(
        "cp4d_watsonstudio_job_schedule_last_run_duration_seconds")
    event_type_watsonstudio_job_schedule_last_run_epoch_time = cp4d_monitor.create_and_validate_type(
        "cp4d_watsonstudio_job_schedule_last_run_epoch_time")
    event_type_watsonstudio_jobs_schedule_user_notebook_count = cp4d_monitor.create_and_validate_type(
        "cp4d_watsonstudio_jobs_schedule_user_notebook_count")
    event_type_watsonstudio_jobs_schedule_user_datastage_count = cp4d_monitor.create_and_validate_type(
    ...
    for project in projects:
        ...
        for job in project_jobs:
            if 'schedule' in job['entity']['job']:
                ...
                if job['entity']['job']['asset_ref_type'] == 'notebook':
                    if project_id in watsonstudio_jobs_schedule_user_notebook_count and job['metadata']['owner_id'] in \
                ...
                if job['entity']['job']['asset_ref_type'] == 'data_intg_flow':
                    if project_id in watsonstudio_jobs_schedule_user_datastage_count and job['metadata']['owner_id'] in \
                ...
                if job_info is not None and 'schedule_info' in job_info['entity']['job']:
                    nextrun_in_epoch_time = cp4d_monitor.calculate_next_schedule_run(
                        job_info['entity']['job']['schedule_info']['startOn'], job_info['entity']['job']['schedule'])
                    if ('endOn' not in job_info['entity']['job']['schedule_info']) or (
                            'endOn' in job_info['entity']['job']['schedule_info'] and nextrun_in_epoch_time * 1000 <=
                            job_info['entity']['job']['schedule_info']['endOn']):
                        events.append({
                            "monitor_type": monitor_type,
                ...
                if len(runs) > 0:
                    if job['metadata']['asset_id'] == runs[0]['entity']['job_run']['job_ref'] and \
                            runs[0]['entity']['job_run']['isScheduledRun'] == True:
                        ...
                        if 'last_state_change_timestamp' in runs[0]['entity']['job_run']:
                            last_run_epoch_time = calendar.timegm(
                                time.strptime(runs[0]['entity']['job_run']['last_state_change_timestamp'],
                                              '%Y-%m-%dT%H:%M:%SZ'))
                            events.append({
                                "monitor_type": monitor_type,
        ...
        events.append({
            ...
            "metadata": metadata_watsonstudio_jobs_schedule_project_count.format(
                watsonstudio_jobs_schedule_project_count),
            "severity": "info",
    ...
    for project_id, _ in watsonstudio_jobs_schedule_user_notebook_count.items():
        for k, v in watsonstudio_jobs_schedule_user_notebook_count[project_id].items():
            events.append({
                ...
                "metadata": metadata_watsonstudio_jobs_schedule_user_notebook_count.format(
                    watsonstudio_jobs_schedule_overall_count),
                "severity": "info",
    ...
    for project_id, _ in watsonstudio_jobs_schedule_user_datastage_count.items():
        for k, v in watsonstudio_jobs_schedule_user_datastage_count[project_id].items():
            events.append({
                ...
                "metadata": metadata_watsonstudio_jobs_schedule_user_datastage_count.format(
                    watsonstudio_jobs_schedule_overall_count),
                "severity": "info",
