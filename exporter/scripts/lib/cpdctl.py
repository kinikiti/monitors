...
def cpdctl_init_config_context(username, password, url, context_name="default"):
    result = cmd_execute(command='cpdctl',
                         parameters=f'config context set {context_name} --username={username} --password={password} --url {url}')
...
def cpdctl_get_job(project_id, job_id, context_name="default"):
    job = cmd_execute(command='cpdctl',
                      parameters=f'job get --project-id {project_id} --job-id {job_id} --context {context_name}')
