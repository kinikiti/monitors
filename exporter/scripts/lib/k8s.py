...
def get_pod_usage(namespace, label_selector):
    resources = custom_api.list_namespaced_custom_object(group="metrics.k8s.io", version="v1beta1",
...
def check_ccs_svc(namespace):
    error_msg = """Custom Resource Definition CCS is either not available or no instance available.
    CPDCTL capabilities are not available on this Cloud Pak for Data instance."""
    result = False
    try:
